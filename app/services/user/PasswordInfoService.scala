...
class PasswordInfoService @javax.inject.Inject() () extends DelegableAuthInfoDAO[PasswordInfo] {
  override def find(loginInfo: LoginInfo) = {
    Future.successful(MasterDatabaseConnection.query(PasswordInfoQueries.getById(Seq(loginInfo.providerID, loginInfo.providerKey))))
  }

  override def add(loginInfo: LoginInfo, authInfo: PasswordInfo) = {
    MasterDatabaseConnection.executeUpdate(PasswordInfoQueries.CreatePasswordInfo(loginInfo, authInfo))
  ...
  override def save(loginInfo: LoginInfo, authInfo: PasswordInfo) = {
    MasterDatabaseConnection.transaction { conn =>
      val rowsAffected = MasterDatabaseConnection.executeUpdate(PasswordInfoQueries.UpdatePasswordInfo(loginInfo, authInfo))
      if (rowsAffected == 0) {
        MasterDatabaseConnection.executeUpdate(PasswordInfoQueries.CreatePasswordInfo(loginInfo, authInfo))
        Future.successful(authInfo)
      } else {
  ...
  override def remove(loginInfo: LoginInfo) = {
    MasterDatabaseConnection.executeUpdate(PasswordInfoQueries.removeById(Seq(loginInfo.providerID, loginInfo.providerKey)))
