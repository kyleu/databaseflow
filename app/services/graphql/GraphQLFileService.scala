...
object GraphQLFileService extends Logging {
  private[this] val dir = new java.io.File(ConfigFileService.configDir, "graphql")

  if (!dir.isDirectory) {
    if (dir.exists) {
      log.error("GraphQL config directory could not be created.")
    } else {
      dir.mkdir
    }
  }
