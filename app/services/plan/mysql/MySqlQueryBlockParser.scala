...
object MySqlQueryBlockParser {
  def parseQueryBlock(depth: Int, json: Js.Value) = {
    val params = json.obj
  ...
  private[this] def parseUnionResult(el: Js.Value) = {
    ...
    val querySpecifications = obj(MySqlParseKeys.keyQuerySpecifications).arr.map(_.obj(MySqlParseKeys.keyQueryBlock))

    val props = JsonUtils.toStringMap(obj).filterNot(_._1 == MySqlParseKeys.keyQuerySpecifications)
    val relation = props("table_name")
    PlanNode(title = "Union Result", nodeType = "Union Result", relation = Some(relation), properties = props)
  }
  ...
  private[this] def parseTable(el: Js.Value) = {
    ...
    val relation = props("table_name")
    val nodeType = props("access_type")
    val rows = obj("rows") match {
      case s: Js.Num => s.value.toInt
      case x => throw new IllegalStateException(x.toString)
    }

    val costs = PlanNode.Costs(actualRows = if (rows != 0) { Some(rows) } else { None })
    PlanNode(title = "Table Scan", nodeType = nodeType, relation = Some(relation), costs = costs, properties = props)
  }
