...
object SandboxGraphQL {
  ...
  val queryFields = fields[GraphQLContext, Unit](Field(
    ...
    description = Some("Returns the list of one-off sandbox tests."),
    resolve = c => SandboxTask.values
  ))

  class SandboxApi(app: ApplicationContext) {
    def call(task: SandboxTask) = task.run(app)
  }
  ...
  val mutationFields = fields[GraphQLContext, Unit](
    Field(
      ...
      resolve = c => new SandboxApi(c.ctx.app)
    )
  )
}
