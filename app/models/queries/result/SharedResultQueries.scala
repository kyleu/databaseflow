...
object SharedResultQueries extends BaseQueries[SharedResult] {
  ...
  case class UpdateSharedResult(sr: SharedResult) extends Statement {
    override val sql = updateSql(Seq(
      ...
      "filter_column", "filter_op", "filter_value", "last_accessed", "created"
    ))
    override val values = Seq[Any](
      ...
      sr.source.filterColumn, sr.source.filterOp, sr.source.filterValue, new java.sql.Timestamp(sr.lastAccessed), new java.sql.Timestamp(sr.created), sr.id
    )
  }
