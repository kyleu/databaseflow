...
class UserLicenseController @javax.inject.Inject() (implicit val messagesApi: MessagesApi, notificationSvc: NotificationService) extends BaseSiteController {
  ...
  def licenseRequest() = act("user-license-request") { implicit request =>
    UserLicenseController.licenseForm.bindFromRequest.fold(
      ...
      f => {
        ...
        notificationSvc.onLicenseCreate(license.id, license.name, license.email, license.edition.title, license.issued, license.version, licenseContent)
