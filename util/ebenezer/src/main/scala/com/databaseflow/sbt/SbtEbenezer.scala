...
class SbtEbenezer extends AutoPlugin {
  // override def requires = ScroogeSBT

  // override lazy val projectSettings = Seq(commands += generateCommand)

  lazy val generateCommand = Command.command("dbfGenerate") { (state: State) =>
    println("Hi there! Starting Database Flow codegen...")
    state
  }
}
