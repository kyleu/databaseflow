...
object LicenseGenerator {
  ...
  def loadLicense(id: String) = {
    ...
    if (Files.exists(file)) {
      ...
      EncryptUtils.decrypt(decoded)
    } else {
      throw new IllegalArgumentException(s"License already exists for [$id].")
    }
  }

  def saveLicense(id: String, content: String, overwrite: Boolean = false) = {
    val filename = id + ".license"
    val file = Paths.get(licenseDir, filename)
    if ((!overwrite) && Files.exists(file)) {
      throw new IllegalArgumentException(s"License already exists for [$id] and cannot be overwritten.")
    } else {
      val encrypted = EncryptUtils.encrypt(content).toArray
