...
object TwirlRelationFiles {
  ...
  private[this] def writeTable(providedPrefix: String, model: ExportModel, refFields: Seq[ExportField], listFile: TwirlFile) = {
    ...
    listFile.add(s"implicit request: Request[AnyContent], session: Session, flash: Flash, traceData: ${providedPrefix}util.tracing.TraceData")
    ...
    listFile.add(s"icon = ${providedPrefix}models.template.Icons.${model.propertyName},")
    ...
    listFile.add(s"calls = ${providedPrefix}models.result.web.ListCalls(", 1)
  ...
  def export(config: ExportConfiguration, model: ExportModel) = {
    model.foreignKeys.flatMap {
      case fk if fk.references.lengthCompare(1) == 0 =>
        ...
        writeTable(config.providedPrefix, model, refFields, listFile)
        Some(listFile)
      case _ => None
    }
  }
