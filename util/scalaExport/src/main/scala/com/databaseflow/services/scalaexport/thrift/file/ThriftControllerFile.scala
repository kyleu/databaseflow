...
object ThriftControllerFile {
  def export(pkg: Seq[String], svc: ThriftService, metadata: ThriftMetadata) = {
    ...
    file.addImport(metadata.depPrefix + "util.FutureUtils", "defaultContext")
    file.addImport(metadata.depPrefix + "util.web.ControllerUtils", "jsonArguments")
    file.addImport(metadata.depPrefix + "util.tracing", "TraceData")
    file.addImport("play.api.mvc", "Call")
    file.addImport("services.thrift", "ThriftServiceRegistry")
    file.addImport(pkg.mkString("."), "_")

    file.add("@javax.inject.Singleton")
    file.add(s"""class ${svc.name}Controller @javax.inject.Inject() (override val app: Application) extends BaseController("${svc.name}") {""", 1)
    file.add(s"val svc = ThriftServiceRegistry.${svc.identifier}")
  ...
  private[this] def addHelpers(svc: ThriftService, file: ScalaFile) = {
    ...
    file.add("val started = util.DateUtils.now")
    ...
    file.add(s"""started = Some(started), completed = Some(util.DateUtils.now), debug = app.config.debug""")
    file.add("""))""", -1)
