...
object ThriftEnumFile {
  def exportString(srcPkg: Seq[String], tgtPkg: Seq[String], model: ThriftStringEnum, exportModelRoot: Option[String]) = {
  ...
  def exportInt(srcPkg: Seq[String], tgtPkg: Seq[String], model: ThriftIntegerEnum, exportModelRoot: Option[String]) = {
    val file = ScalaFile(pkg = tgtPkg, key = model.name, root = exportModelRoot)
