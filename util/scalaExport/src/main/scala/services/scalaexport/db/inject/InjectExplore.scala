  ...
  def injectMenu(result: ExportResult, rootDir: File) = {
    def queryFieldsFor(s: String) = {
      ...
      val newContent = (roots ++ pkgGroups.flatMap(_._2)).sortBy(_.title).map { model =>
        s"""  <li><a href="@${model.routesClass}.list()">${model.iconHtml(result.config.rootPrefix)} ${model.title}</a></li>"""
      }.mkString("\n")
    ...
    def queryFieldsFor(s: String) = {
      ...
      val newContent = (roots ++ pkgGroups.flatMap(_._2)).sortBy(_.title).map { model =>
        s"""    <li class="collection-item">
          |      <a class="theme-text" href="@${model.routesClass}.list()">${model.iconHtml(result.config.rootPrefix)} ${model.plural}</a>
