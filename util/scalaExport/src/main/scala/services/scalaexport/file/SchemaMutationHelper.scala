...
object SchemaMutationHelper {
  def addMutationFields(model: ExportModel, file: ScalaFile) = if (model.pkFields.nonEmpty) {
    ...
    file.add(s"""traceB(c.ctx, "create")(tn => c.ctx.${model.serviceReference}.create(c.ctx.creds, dataFields)(tn))""")
    file.add("}", -1)
    file.add("),", -1)
    ...
    file.add(s"resolve = c => {", 1)
    file.add("val dataFields = c.args.arg(DataFieldSchema.dataFieldsArg)")
    file.add(s"""traceB(c.ctx, "update")(tn => c.ctx.${model.serviceReference}.update(c.ctx.creds, $argProps, dataFields)(tn)._1)""")
    file.add("}", -1)
    file.add("),", -1)
    ...
    file.add(s"fieldType = ${model.propertyName}Type,")
    file.add(s"""resolve = c => traceB(c.ctx, "remove")(tn => c.ctx.${model.serviceReference}.remove(c.ctx.creds, $argProps)(tn))""")
    file.add(")", -1)
