...
object ServiceHelper {
  def writeSearchFields(model: ExportModel, file: ScalaFile, queriesFile: String, trace: String, searchArgs: String) = {
    file.add(s"override def countAll(user: User, filters: Seq[Filter] = Nil)$trace = {", 1)
    file.add(s"""traceB("get.all.count")(td => ApplicationDatabase.query($queriesFile.countAll(filters))(td))""")
    file.add("}", -1)
    file.add(s"override def getAll(user: User, $searchArgs)$trace = {", 1)
    file.add(s"""traceB("get.all")(td => ApplicationDatabase.query($queriesFile.getAll(filters, orderBys, limit, offset))(td))""")
    file.add("}", -1)
    file.add()
    file.add("// Search")
    file.add(s"override def searchCount(user: User, q: String, filters: Seq[Filter])$trace = {", 1)
    ...
    file.add(s"  user: User, q: String, $searchArgs")
    file.add(s")$trace = {", 1)
    file.add(s"""traceB("search")(td => ApplicationDatabase.query($queriesFile.search(q, filters, orderBys, limit, offset))(td))""")
    file.add("}", -1)
    file.add()
    file.add(s"def searchExact(user: User, q: String, orderBys: Seq[OrderBy] = Nil, limit: Option[Int] = None, offset: Option[Int] = None)$trace = {", 1)
  ...
  def addGetters(model: ExportModel, file: ScalaFile) = {
    ...
    model.pkFields match {
      ...
      case field :: Nil =>
        ...
        file.add(s"def getByPrimaryKey(user: User, $colProp: ${field.t.asScala})$td = {", 1)
        ...
        file.add(s"def getByPrimaryKeySeq(user: User, $seqArgs)$td = {", 1)
      ...
      case fields => // multiple columns
        ...
        file.add(s"def getByPrimaryKey(user: User, $colArgs)$td = {", 1)
        file.add(s"""traceB("get.by.primary.key")(td => ApplicationDatabase.query(${model.className}Queries.getByPrimaryKey($queryArgs))(td))""")
        file.add("}", -1)

        file.add(s"def getByPrimaryKeySeq(user: User, pkSeq: Seq[$tupleTyp])$td = {", 1)
  ...
  def writeForeignKeys(model: ExportModel, file: ScalaFile) = model.foreignKeys.foreach { fk =>
    ...
    fk.references.toList match {
      case h :: Nil =>
        ...
        file.add(s"""def countBy$propCls(user: User, $propId: ${col.t.asScala})(implicit trace: TraceData) = traceB("count.by.$propId") { td =>""", 1)
        file.add(s"ApplicationDatabase.query(${model.className}Queries.CountBy$propCls($propId))(td)")
        file.add("}", -1)
        file.add(s"def getBy$propCls(user: User, $propId: ${col.t.asScala}, $searchArgs)(implicit trace: TraceData) = {", 1)
        ...
        file.add("}", -1)
        file.add(s"""def getBy${propCls}Seq(user: User, ${propId}Seq: Seq[${col.t.asScala}])(implicit trace: TraceData) = traceB("get.by.$propId.seq") { td =>""", 1)
