...
object ControllerFile {
  def export(config: ExportConfiguration, model: ExportModel) = {
    ...
    file.add("svc.create(request.identity, modelForm(request.body.asFormUrlEncoded)) match {", 1)
    ...
    file.add("case Some(query) if query.nonEmpty => svc.searchWithCount(request.identity, query, Nil, orderBys, limit.orElse(Some(100)), offset)")
    file.add("case _ => svc.getAllWithCount(request.identity, Nil, orderBys, limit.orElse(Some(100)), offset)")
    file.add("}", -1)
    ...
    file.add("val orderBys = OrderBy.forVals(orderBy, orderAsc).toSeq")
    file.add("val r = q match {", 1)
    file.add("case Some(query) if query.nonEmpty => svc.search(request.identity, query, Nil, orderBys, limit.orElse(Some(5)), None)")
    file.add("case _ => svc.getAll(request.identity, Nil, orderBys, limit.orElse(Some(5)))")
    file.add("}", -1)
