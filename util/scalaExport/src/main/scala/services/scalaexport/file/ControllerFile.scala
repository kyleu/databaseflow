...
object ControllerFile {
  def export(config: ExportConfiguration, model: ExportModel) = {
    ...
    file.add("case Some(query) if query.nonEmpty => svc.searchWithCount(request, query, Nil, orderBys, limit.orElse(Some(100)), offset)")
    file.add("case _ => svc.getAllWithCount(request, Nil, orderBys, limit.orElse(Some(100)), offset)")
    file.add("}", -1)
    file.add("f.map(r => render {", 1)
    ...
    file.add("val orderBys = OrderBy.forVals(orderBy, orderAsc).toSeq")
    file.add("val f = q match {", 1)
    file.add("case Some(query) if query.nonEmpty => svc.search(request, query, Nil, orderBys, limit.orElse(Some(5)), None)")
    file.add("case _ => svc.getAll(request, Nil, orderBys, limit.orElse(Some(5)))")
    file.add("}", -1)
    file.add("f.map(r => Ok(r.map(_.toSummary).asJson.spaces2).as(JSON))")
    file.add("}", -1)
    file.add("}", -1)
