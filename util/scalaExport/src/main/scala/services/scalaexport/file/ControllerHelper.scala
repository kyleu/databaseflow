...
object ControllerHelper {
  ...
  def writePks(model: ExportModel, file: ScalaFile, viewPkg: String, routesClass: String) = if (model.pkFields.nonEmpty) {
    ...
    file.add(s"val f = svc.getByPrimaryKey(request, $getArgs)")
    file.add(s"""app.noteService.getFor("${model.propertyName}", ${model.pkFields.map(_.propertyName).mkString(", ")}).flatMap { notes =>""", 1)
    file.add(s"f.map {", 1)
