...
object ControllerHelper {
  ...
  def writePks(model: ExportModel, file: ScalaFile, viewPkg: String, routesClass: String) = if (model.pkFields.nonEmpty) {
    ...
    file.add(s"""val notes = app.noteService.getFor("${model.propertyName}", ${model.pkFields.map(_.propertyName).mkString(", ")})""")
    file.add(s"svc.getByPrimaryKey(request.identity, $getArgs) match {", 1)
    file.add("case Some(model) => Future.successful(render {", 1)
    file.add(s"case Accepts.Html() => Ok($viewPkg.${model.propertyName}View(request.identity, model, notes, app.config.debug))")
    file.add("case Accepts.Json() => Ok(model.asJson.spaces2).as(JSON)")
    file.add("})", -1)
    file.add(s"""case None => Future.successful(NotFound(s"No ${model.className} found with $getArgs [$logArgs]."))""")
    file.add("}", -1)
    file.add("}", -1)
    file.add()
    ...
    file.add(s"svc.getByPrimaryKey(request.identity, $getArgs) match {", 1)
    file.add(s"case Some(model) => Future.successful(Ok(", 1)
    ...
    file.add(s"val res = svc.update(request.identity, $callArgs, fields = fields)")
    ...
    file.add(s"svc.remove(request.identity, $callArgs)")
    file.add("Future.successful(render {", 1)
  ...
  def writeForeignKeys(model: ExportModel, file: ScalaFile) = model.foreignKeys.foreach { fk =>
    fk.references.toList match {
      case h :: Nil =>
        ...
        file.add(s"val models = svc.getBy$propCls(request.identity, $propId, orderBys, limit, offset)")
        file.add("Future.successful(render {", 1)
