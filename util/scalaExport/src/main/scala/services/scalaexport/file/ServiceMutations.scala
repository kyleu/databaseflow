...
object ServiceMutations {
  ...
  def mutations(model: ExportModel, file: ScalaFile) = {
    if (model.pkFields.nonEmpty) {
      ...
      file.add()
      file.add(s"def remove(user: User, $sig)$trace = {", 1)
      file.add(s"""traceB("remove")(td => getByPrimaryKey(user, $call)(td) match {""", 1)
      ...
      file.add("}", -1)
      file.add()
      file.add(s"def update(user: User, $sig, fields: Seq[DataField])$trace = {", 1)
      file.add(s"""traceB("update")(td => getByPrimaryKey(user, $call)(td) match {""", 1)
      ...
      file.add(s"getByPrimaryKey(user, $call)(td) match {", 1)
