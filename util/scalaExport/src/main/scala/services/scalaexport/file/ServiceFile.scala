...
object ServiceFile {
  ...
  def export(model: ExportModel) = {
    ...
    file.add(s"""override def countAll(filters: Seq[Filter] = Nil)$trace = traceF("get.all.count") { td =>""", 1)
    file.add(s"Database.query(${model.className}Queries.countAll(filters))(td)")
    file.add("}", -1)
    file.add(s"""override def getAll($searchArgs)$trace = traceF("get.all") { td =>""", 1)
    file.add(s"Database.query(${model.className}Queries.getAll(filters, orderBys, limit, offset))(td)")
    file.add("}", -1)
    file.add()
    file.add(s"""override def searchCount(q: String, filters: Seq[Filter])$trace = traceF("search.count") { td =>""", 1)
    file.add(s"Database.query(${model.className}Queries.searchCount(q, filters))(td)")
    file.add("}", -1)
    file.add(s"""override def search(q: String, $searchArgs)$trace = traceF("search") { td =>""", 1)
    file.add(s"Database.query(${model.className}Queries.search(q, filters, orderBys, limit, offset))(td)")
    file.add("}", -1)
    file.add()
    file.add(s"""def searchExact(q: String, orderBys: Seq[OrderBy], limit: Option[Int], offset: Option[Int])$trace = traceF("search.exact") { td =>""", 1)
    file.add(s"Database.query(${model.className}Queries.searchExact(q, orderBys, limit, offset))(td)")
    file.add("}", -1)
    file.add()

    ForeignKeysHelper.writeService(model, file)
    file.add(s"""def insert(model: ${model.className})$trace = traceF("insert") { td =>""", 1)
    file.add(s"Database.execute(${model.className}Queries.insert(model))(td).flatMap {", 1)
    ...
    file.add(s"""def create(fields: Seq[DataField])$trace = traceF("create") { td =>""", 1)
    file.add(s"Database.execute(${model.className}Queries.create(fields))(td).map { _ =>", 1)
