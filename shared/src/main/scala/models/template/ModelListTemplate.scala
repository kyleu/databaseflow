...
object ModelListTemplate {
  def forSavedQueries(queryId: UUID, savedQueries: Seq[SavedQuery]) = {
    ...
    forModels(queryId, "saved-query", "Saved Queries", tableFor(cols, None, rows))
  }
  ...
  def forViews(queryId: UUID, views: Seq[View]) = {
    ...
    forModels(queryId, "view", "Views", tableFor(cols, None, rows))
  }
  ...
  def forProcedures(queryId: UUID, procedures: Seq[Procedure]) = {
    ...
    forModels(queryId, "procedure", "Procedures", tableFor(cols, None, rows))
  }

  private[this] def tableFor(cols: Seq[String], sortKey: Option[String], rows: Seq[TypedTag[String]]) = {
