...
object SidenavTemplate {
  ...
  private[this] def table(t: String) = li(a(id := "sidenav-table-" + t, cls := "sidenav-link waves-effect waves-light", href := "#table-" + t)(
    em(cls := s"fa ${Icons.tableClosed}"),
    span(title := t)(t)
  ))
  def tables(tables: Seq[String]) = tables.map(table)

  private[this] def view(v: String) = li(a(id := "sidenav-view-" + v, cls := "sidenav-link waves-effect waves-light", href := "#view-" + v)(
    em(cls := s"fa ${Icons.view}"),
    span(title := v)(v)
  ))
  def views(views: Seq[String]) = views.map(view)

  private[this] def procedure(p: String) = li(a(id := "sidenav-procedure-" + p, cls := "sidenav-link waves-effect waves-light", href := "#procedure-" + p)(
